"""Real-model inference: the model-independent pieces (no `ultralytics`/`torch`).
...
The YOLO-backed builders (`build_local_inference_fn`/
`build_offload_inference_fn`) that actually load a model and run real
inference live in `datagen.simulate.yolo_inference`, not here — this module
stays free of the `ultralytics`/`torch` import cost so callers that only
need `DetectionResult`/`score_accuracy`/`apply_condition_overhead` (e.g.
`datagen.persistence`) don't pay for it.
"""
