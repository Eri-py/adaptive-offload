...
- Network and device conditions are simulated in-process — latency is
  modeled analytically as a function of each condition vector's
  bandwidth/latency/packet-loss or device-load value (not by sleeping in
  real time; simulating real delay across 25,000 rows would cost hours of
  wall clock without changing any of the resulting data) — no real traffic
  shaping or OS-level load injection.
